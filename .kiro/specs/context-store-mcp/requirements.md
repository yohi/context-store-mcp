...
4. WHEN データの削除が要求されたとき THEN Context Store MCPシステム SHALL 以下の安全な削除手順を実行する:
   - **削除手順:**
     ...
     4. **バックアップからの削除:** 削除から365日後にバックアップから該当データを除外（バックアップの再作成または削除マーク）
   - **復元不可能性の保証:**
   ...
   - **保持期間の整合性:**
     - バックアップ保持期間（365日）は監査ログ保持期間と一致し、監査証跡の検証可能性を保証
     - 削除から365日以内のすべての削除イベントについて、監査ログとバックアップの両方から検証可能
   - **受け入れテスト基準:**
     - 削除後にデータベースクエリで該当記憶が取得できないことを確認
     - バックアップからのリストアテストで削除済みデータが復元されないことを確認（削除から365日後）
     - 鍵破棄後に暗号化データの復号化試行が失敗することを確認
     - 削除から1年以内の任意の時点で、監査ログとバックアップの両方から削除イベントを検証できることを確認
...
5. WHILE システムが稼働している間 THE Context Store MCPシステム SHALL 以下の監査ログを維持する:
   ...
   - **必須ログフィールド:**
     ...
     ```
  - **検索/クエリSLA:**
    - **過去30日間（ホットストレージ）:** 5秒以内に検索可能（Elasticsearchまたは同等のログ分析基盤で実現）
    - **31日～365日（コールドストレージ）:** 30秒以内に検索可能（AWS S3 Glacier、Azure Archive Storage、またはPostgreSQLのパーティション分割された古いログテーブルから検索）
    - **ストレージ階層化:** 30日経過後のログは自動的にコールドストレージに移行し、コスト効率を最適化しつつ長期保持要件を満たす
   - **アクセス履歴追跡:**
     - 各記憶（`memory_id`）ごとにアクセス履歴を`access_history`テーブルで管理
     - 最終アクセス日時（`last_accessed_at`）およびアクセス回数（`access_count`）を記録
   - **受け入れテスト基準:**
    - 100日前のログエントリを30秒以内に検索できることを確認（95パーセンタイル）
